...
@csrf_exempt
def process(request):
    dbconn = pymysql.connect(host=host, database=database,
                             user=user, password=password, port=port, charset=charset)
    ...
    try:
        if process == 1:
            cursor.execute("UPDATE t_ea_evaluate_revise_detial set is_revise=1 where shop_id = " + shopid + " and phones like '%" + phone +
                           "%' and order_id =" + orderid+";UPDATE t_ea_evaluate_revise SET left_counter = left_counter - 1 where shop_id= "+shopid + ";")
            print("Shopid=" + shopid + " Phone=" + phone + " Success.")
        elif process == 2:
            cursor.execute("UPDATE t_ea_evaluate_revise_detial set is_revise=2 where shop_id = " +
                           shopid + " and phones like '%" + phone + "%' and order_id =" + orderid+";")
            print("Shopid=" + shopid + " Phone=" + phone + " Fail.")
        elif process == 3:
            cursor.execute("UPDATE t_ea_evaluate_revise_detial set is_revise=1 where shop_id = " +
                           shopid + " and phones like '%" + phone + "%' and order_id =" + orderid+";")
...
def index(request):
    ...
    for row in results:
        ...
        if row[20] > datetime.now() - timedelta(days=7):
            ...
            expired = True
        table.append({'shopid': row[1], 'evaluateid': row[2], 'beforerating': row[4], 'beforecontent': row[5], 'afterrating': row6, 'aftercontent': row7, 'createtime': row[8], 'revisetime': row[9], 'creator': row[10], 'reviser': row11,
                      'isrevise': row12, 'buyer': row[13], 'phone': row[14], 'beforetime': row[15], 'aftertime': row[17], 'ordertime': row[20], 'appid': row[21], 'orderid': row[22], 'isrefund': row[23], 'leftcount': row[24], 'expired': expired})
    cursor.execute(
    ...
    print("Total count :" + str(remaincount[0][0]) +
          str(successcount[0][0]) + str(failcount[0][0]))
...
def remind(request):
    ...
    orderid = request.POST.get('orderid')
    content = shopname[:int(shopname.index("<br>"))] + '于' + createtime + '提交的差评代改服务（来自顾客' + customer + \
        ' ' + phone + '）小评果客服已经与顾客取得联系并协商修改。最终结果以顾客实际操作为准。说明：差评协商修改并非100%能成功，目前总体成功率60%左右，感谢理解！'
    cursor.execute(
        "SELECT phone FROM `t_phone` where shop_id=" + shopid + ";")
    results = cursor.fetchall()
    for row in results:
        # print(row[0])
        sendmsg(row[0], content)
    try:
        cursor.execute("UPDATE t_ea_evaluate_revise_detial set reviser='admin' where shop_id =" + shopid + " and phones like '%" + phone + "%' and order_id =" + orderid + ";")
        dbconn.commit()
    except:
        print("Process fail. Rollback.")
        dbconn.rollback()
